...
from app.schemas.module_bucket import BucketInfo
...
async def module_new_bucket(module_name: str, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    module = await db.execute(
        select(Module).options(selectinload(Module.bucket)).where(Module.name == module_name)
    )
    module = module.scalar_one_or_none()
...
@router.get("/bucket", response_model=BucketInfo)
...
async def module_put_bucket(module_name: str, bucket_info: BucketInfo, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    module = await get_module(module_name, db)
