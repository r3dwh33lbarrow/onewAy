...
class TestingSettings(BaseSettings):
    ...
    database: Optional[TestingDatabaseSettings] = None
    security: Optional[TestingSecuritySettings] = None
    paths: Optional[TestingPathsSettings] = None
...
class Settings(BaseSettings):
    ...
    def _testing_check(self) -> "Settings":
        ...
        if self.testing is not None and self.testing.testing:
            if self.testing.database is None or not self.testing.database.url:
                raise RuntimeError(
                    "[ERROR in config.yaml] You must provide a testing database URL if testing"
                )
            if self.testing.security is None or not self.testing.security.algorithm:
                raise RuntimeError(
    ...
    def inject_testing(self) -> "Settings":
        ...
        if self.testing is not None and self.testing.testing:
            if self.testing.database is not None:
            ...
            if self.testing.security is not None:
