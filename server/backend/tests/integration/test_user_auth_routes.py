...
async def test_user_register_and_login(client: AsyncClient):
    r = await client.post(
        "/user/auth/register", json={"username": "alice", "password": "pw"}
    )
    assert r.status_code == 200
    assert r.json() == {"result": "success"}

    r = await client.post(
        "/user/auth/login", json={"username": "alice", "password": "pw"}
    )
    assert r.status_code == 200
    assert r.json() == {"result": "success"}
...
async def test_user_login_wrong_password(client: AsyncClient):
    ...
    r = await client.post(
        "/user/auth/login", json={"username": "bob", "password": "wrong"}
    )
...
async def test_logout_clears_cookie(client: AsyncClient):
    await client.post(
        "/user/auth/register", json={"username": "dave", "password": "pw"}
    )
    r = await client.post(
        "/user/auth/login", json={"username": "dave", "password": "pw"}
    )
    assert "access_token" in r.cookies
...
async def test_user_auth_ws_token(client: AsyncClient):
    await client.post(
        "/user/auth/register", json={"username": "wsuser2", "password": "pw"}
    )
    r = await client.post(
        "/user/auth/login", json={"username": "wsuser2", "password": "pw"}
    )
    assert r.status_code == 200
