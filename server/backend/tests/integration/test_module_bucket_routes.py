...
async def _user_headers(client: AsyncClient, username: str = "admin", password: str = "pw"):
    await client.post(
...
async def test_module_bucket_lifecycle(client: AsyncClient, db_session: AsyncSession):
    ...
    # Create new bucket
    r = await client.post("/module/new-bucket", params={"module_name": "bucket_mod"}, headers=headers)
    assert r.status_code == 200
    assert r.json() == {"result": "success"}

    # Initially empty data
    r = await client.get("/module/bucket", params={"module_name": "bucket_mod"}, headers=headers)
    assert r.status_code == 200
    assert r.json() == {"data": ""}
...
@pytest.mark.asyncio
async def test_module_bucket_conflicts_and_errors(client: AsyncClient, db_session: AsyncSession):
    # Create a module
    m = Module(
        name="conflict_mod",
    ...
    r = await client.post("/module/new-bucket", params={"module_name": "conflict_mod"}, headers=headers)
    assert r.status_code == 200
    ...
    r = await client.post("/module/new-bucket", params={"module_name": "no_such"}, headers=headers)
    assert r.status_code == 404
    assert r.json()["detail"] == "Module not found"

    # Unknown module returns 404 for get
    r = await client.get("/module/bucket", params={"module_name": "no_such"}, headers=headers)
    assert r.status_code == 404
    assert r.json()["detail"] == "Module not found"


@pytest.mark.asyncio
async def test_module_bucket_get_without_existing_bucket(client: AsyncClient, db_session: AsyncSession):
    ...
    r = await client.get("/module/bucket", params={"module_name": "nobucket_mod"}, headers=headers)
    assert r.status_code == 400
    assert r.json()["detail"] == "No bucket exists for module"
