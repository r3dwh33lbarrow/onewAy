...
async def test_user_get_default_avatar(client: AsyncClient):
    ...
    response = await client.get("/user/avatars")
    assert response.status_code == 200
...
async def test_user_put_avatar_success(client: AsyncClient):
    ...
    files = {"file": ("avatars.png", avatar_content, "image/png")}
    response = await client.put("/user/avatars", files=files)
    assert response.status_code == 200
    assert response.json() == {"result": "success"}
    ...
    avatar_response = await client.get("/user/avatars")
