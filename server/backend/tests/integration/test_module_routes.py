...
async def _client_headers(
    client: AsyncClient, username="modclient", password="pw", version="1.0.0"
):
    await client.post(
        "/client/auth/enroll",
...
async def test_module_upload_and_query_dir(client: AsyncClient):
    ...
    files = {
        "files": ("config.yaml", b"name: test_upload\nversion: 0.1.0\nstart: manual\n")
    }
...
async def test_module_run_and_delete(client: AsyncClient, db_session: AsyncSession):
    ...
    await client.post(
        "/module/set-installed/runner",
        params={"module_name": "run_delete"},
        headers=client_headers,
    )

    r = await client.get(
        "/module/run/run_delete",
        params={"client_username": "runner"},
...
async def test_module_add_from_local_path_and_list_all(client: AsyncClient):
    ...
    r = await client.put(
        "/module/add", headers=user_headers, json={"module_path": "test_module"}
    )
    assert r.status_code == 200
    assert r.json() == {"result": "success"}
...
async def test_module_add_nonexistent_path_returns_400(client: AsyncClient):
    ...
    r = await client.put(
        "/module/add", headers=user_headers, json={"module_path": "does_not_exist_dir"}
    )
    assert r.status_code == 400
...
async def test_module_update_not_found_returns_404(client: AsyncClient):
    ...
    r = await client.put(
        "/module/update/no_such_module", headers=user_headers, files=files
    )
...
async def test_module_installed_list_includes_set_item(
    ...
    r = await client.post(
        "/module/set-installed/instclient",
