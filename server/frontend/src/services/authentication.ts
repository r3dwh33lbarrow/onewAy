...
export async function Register(data: { username: string, password: string }): Promise<boolean> {
  const response = await apiClient.post<typeof data, BasicTaskResponse>('/user/auth/register', data);
  return 'result' in response;
}